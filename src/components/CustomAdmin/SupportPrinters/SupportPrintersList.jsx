...
function SupportPrintersList() {
  ...
  const fetchPrinters = async () => {
    try {
      setLoading(true)
      const response = await fetch("http://localhost:5000/api/imp_support")
      if (!response.ok) {
        throw new Error("Erreur lors du chargement des imprimantes support")
      }
  ...
  if (loading) {
    return (
      <div className="page-loading">
        ...
        <p>Chargement des imprimantes support...</p>
      </div>
    )
  }
  ...
  return (
    <div className="support-printers-list">
      <div className="page-header">
        <h1>Imprimantes Support</h1>
        <Link to="/support-printers/new" className="btn-primary">
          ➕ Nouvelle imprimante support
        </Link>
      </div>
      ...
      <div className="results-info">
        <p>{filteredPrinters.length} imprimante(s) support trouvée(s)</p>
      </div>
              ...
              <tr key={printer.id_eqts}>
                ...
                <td>
                  <div className="action-buttons">
                    <Link to={`/support-printers/${printer.id_eqts}/edit`} className="btn-edit" title="Modifier">
                      ✏️
                    </Link>
      ...
      {filteredPrinters.length === 0 && (
        <div className="no-results">
          <p>Aucune imprimante support trouvée</p>
          {searchTerm && (
...
export default SupportPrintersList
